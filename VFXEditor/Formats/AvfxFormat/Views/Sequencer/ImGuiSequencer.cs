...
namespace VfxEditor.AvfxFormat {
    public abstract class ImGuiSequencer<T> : IUiItem where T : class, IIndexUiItem {
        ...
        public void Draw() {
            ...
            drawList.AddRectFilled( canvasPos, canvasPos + headerSize, 0xFFFF0000, ImDrawFlags.None );
            ...
            drawList.AddRectFilled( canvasPos, canvasPos + canvasSize, 0xFF242424, ImDrawFlags.None );

            drawList.AddRectFilled( canvasPos, new Vector2( canvasPos.X + canvasSize.X, canvasPos.Y + ItemHeight ), 0xFF3D3837, ImDrawFlags.None );
            ...
            for( var i = 0; i < count; i++ ) {
                ...
                if( cursorY >= itemPosBase.Y && cursorY < itemPos.Y + ItemHeight && MovingEntry == null && cursorX > contentMin.X && cursorX < contentMin.X + canvasSize.X ) {
                    ...
                    drawList.AddRectFilled( itemPosBase + new Vector2( 0, 1 ), itemPosBase + new Vector2( LegendWidth, ItemHeight ), color, ImDrawFlags.None );
                }
                ...
                if( !newItemAdded && overCheck && ImGui.IsMouseClicked( ImGuiMouseButton.Left ) && UiUtils.MouseOver( childFramePos, childFramePosMax ) && focused ) {
                    ignoreSelected = true;
                    Toggle( item );
                }

                var overDelete = AddDeleteButton( drawList, new Vector2( contentMin.X + LegendWidth - ItemHeight + 2 - 10, itemPos.Y ), false );
                if( !newItemAdded && overDelete && ImGui.IsMouseClicked( ImGuiMouseButton.Left ) && UiUtils.MouseOver( childFramePos, childFramePosMax ) && focused ) {
                    ignoreSelected = true;
                    deleteEntry = item;
                }

                if( !newItemAdded && !ignoreSelected && UiUtils.MouseOver( itemPosBase, itemPosBase + new Vector2( 150, ItemHeight ) ) && UiUtils.MouseOver( childFramePos, childFramePosMax ) && focused ) {
            ...
            for( var i = 0; i < count; i++ ) {
                ...
                drawList.AddRectFilled( itemPos, itemSize, color, ImDrawFlags.None );
            }
            ...
            for( var i = 0; i < count; i++ ) {
                ...
                if( MovingEntry == null ) {
                    for( var j = 2; j >= 0; j-- ) {
                        var rect = rects[j];
                        if( !UiUtils.MouseOver( rect.Min, rect.Max ) ) continue;
                    ...
                    for( var j = 0; j < 3; j++ ) {
                        var rect = rects[j];
                        if( !UiUtils.MouseOver( rect.Min, rect.Max ) ) continue;
                        if( !UiUtils.MouseOver( childFramePos, childFramePos + childFrameSize ) ) continue;
            ...
            drawList.AddRectFilled( scrollBarA, scrollBarB, 0xFF222222, ImDrawFlags.None );

            var inScrollBar = UiUtils.MouseOver( scrollBarA, scrollBarB );
            drawList.AddRectFilled( scrollBarA, scrollBarB, 0xFF101010, ImDrawFlags.None );
            ...
            drawList.AddRectFilled( scrollBarC, scrollBarD, ( inScrollBar || MovingScrollBar ) ? 0xFF606060 : 0xFF505050, ImDrawFlags.None );
            ...
            var onLeft = UiUtils.MouseOver( barHandleLeft.Min, barHandleLeft.Max );
            var onRight = UiUtils.MouseOver( barHandleRight.Min, barHandleRight.Max );
            ...
            if( SizingRight ) {
                ...
                if( MovingScrollBar ) {
                    ...
                    if( UiUtils.MouseOver( scrollBarThumb.Min, scrollBarThumb.Max ) && ImGui.IsMouseClicked( ImGuiMouseButton.Left ) && MovingEntry == null ) {
